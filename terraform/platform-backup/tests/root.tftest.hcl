...
run "inventory_identity_cannot_be_overridden" {
  command = plan
  variables {
    tags = { PlatformInstanceId = "commerce-123", Owner = "platform-sre", CostCenter = "cc-123", AccountId = "999999999999", Region = "us-east-1", Layer = "network" }
  }
  assert {
    condition     = try(output.backup.tags.AccountId == "123456789012" && output.backup.tags.Region == "ap-northeast-2" && output.backup.tags.Project == "commerce" && output.backup.tags.Layer == "platform-backup", false)

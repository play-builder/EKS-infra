...
class Bootstrap(unittest.TestCase):
    ...
    def test_real_bootstrap_boundaries_keep_passwords_out_of_argv_and_isolate_roles(self):
        ...
        self.assertNotIn('ALTER ROLE commerce_runtime LOGIN NOSUPERUSER',sql[0])
        transfer=sql[0].index('ALTER SCHEMA public OWNER TO commerce_migration')
        self.assertLess(sql[0].index('GRANT CREATE ON DATABASE "commerce" TO commerce_migration'),transfer)
        self.assertLess(sql[0].index('GRANT commerce_migration TO "platform_admin" WITH SET TRUE'),transfer)
        self.assertGreater(sql[0].index('SET LOCAL ROLE commerce_migration'),transfer)
        self.assertGreater(sql[0].index('REVOKE CREATE ON DATABASE "commerce" FROM commerce_migration'),transfer)
        self.assertGreater(sql[0].index('GRANT commerce_migration TO "platform_admin" WITH SET FALSE'),transfer)
        self.assertIn('GRANT USAGE ON SCHEMA public TO "platform_admin"',sql[0])

...
jq '.profile.expiresAt="2099-02-31T00:00:00Z"' \
  "$root/tests/fixtures/prod-live-capacity-go.json" >"$tmp_dir/invalid-time-live.json"
...
if [[ -s "$tmp_dir/invalid-time-aws.log" || -s "$tmp_dir/invalid-time-kube.log" || -e "$tmp_dir/invalid-time-result.json" ]]; then
  echo 'invalid live capacity profile timestamp must fail before cloud queries or evidence output' >&2
  exit 1
fi

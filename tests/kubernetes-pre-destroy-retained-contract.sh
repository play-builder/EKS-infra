...
COURSE_CHECK_BIN_DIR="$tmp_dir/bin" COURSE_ID=course-2026 AWS_ACCOUNT_ID=123456789012 AWS_REGION=ap-northeast-2 \
...
jq -e '[.retainedStorage[] | .kind] | sort == ["Namespace","PersistentVolumeClaim","VolumeSnapshot"]' \
  "$tmp_dir/pre-destroy.json" >/dev/null

set +e
FAKE_EXTRA=true COURSE_CHECK_BIN_DIR="$tmp_dir/bin" COURSE_ID=course-2026 AWS_ACCOUNT_ID=123456789012 AWS_REGION=ap-northeast-2 \
  bash "$root/scripts/kubernetes-pre-destroy-scan.sh" \
    --inventory "$tmp_dir/inventory.json" --gitops-removal "$tmp_dir/removal.json" \

...
rg -q 'CLEANUP_PLAN_ACTION_NOT_ALLOWED' "$root/scripts/final-cleanup.sh"
rg -q 'expected_backend_key_for_root' "$root/scripts/final-cleanup.sh"
rg -q 'OPERATOR_ACCESS_ROLE_TRUST_INVALID' "$root/modules/compute/operator-access/variables.tf"
rg -q 'aws_eks_access_entry' "$root/modules/compute/operator-access/main.tf"

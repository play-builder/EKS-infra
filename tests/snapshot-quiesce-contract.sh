...
output=$(PLATFORM_CHECK_BIN_DIR="$tmp_dir" bash "$root/scripts/snapshot-quiesce-check.sh" "$fixtures/snapshot-quiesce-valid.json")
...
reject() {
  ...
  set +e
  PLATFORM_CHECK_BIN_DIR="$tmp_dir" bash "$root/scripts/snapshot-quiesce-check.sh" "$fixtures/$fixture" >/dev/null 2>&1
  status=$?
  set -e
...
reject_file() {
  ...
  set +e
  PLATFORM_CHECK_BIN_DIR="$tmp_dir" bash "$root/scripts/snapshot-quiesce-check.sh" "$file" >/dev/null 2>&1
  status=$?
  set -e
...
set +e
PLATFORM_CHECK_BIN_DIR="$tmp_dir" bash "$root/scripts/snapshot-quiesce-check.sh" "$tmp_dir/extra.json" >/dev/null 2>&1
status=$?
set -e
[[ "$status" -ne 0 ]]
...
for cluster_length in 1 100; do
  ...
  PLATFORM_CHECK_BIN_DIR="$tmp_dir" bash "$root/scripts/snapshot-quiesce-check.sh" "$candidate" >/dev/null
done

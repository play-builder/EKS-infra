...
class BackupContract(unittest.TestCase):
    ...
    def test_rehydration_rejects_empty_and_stale_conditions_without_secret_reads(self):
        ...
        items=[{'metadata':{'name':n,'generation':2},'status':{'conditions':[{'type':'Ready','status':'True','observedGeneration':2}]}} for n in names]
        def run(args,**kwargs):
        ...
        items[0]['status']['conditions'][0]['observedGeneration']=1
    ...
    def test_restore_requires_different_cluster_and_live_healthy_revision(self):
        ...
        app={'metadata':{'name':'mini-commerce-prod'},'status':{'sync':{'status':'Synced','revision':'a'*40},'health':{'status':'Healthy'}}}
        current=dt.datetime(2026,9,5,1,tzinfo=dt.timezone.utc)
        result=backup.verify_restore(metadata,'arn:aws:eks:us-east-1:123456789012:cluster/recovery','argocd','3.5.2',[app],current)
        ...
        for args in [(metadata['source']['clusterArn'],'argocd','3.5.2',[app]),('arn:aws:eks:us-east-1:123456789012:cluster/recovery','argocd','3.5.1',[app]),('arn:aws:eks:us-east-1:123456789012:cluster/recovery','argocd','3.5.2',[])]:
            with self.assertRaises(backup.Denied): backup.verify_restore(metadata,*args,current)
        bad=copy.deepcopy(app);bad['status']['sync']['revision']='b'*40
        with self.assertRaises(backup.Denied): backup.verify_restore(metadata,'arn:aws:eks:us-east-1:123456789012:cluster/recovery','argocd','3.5.2',[bad],current)

...
case "$*" in
  ...
  *"get crd podchaos.chaos-mesh.org networkchaos.chaos-mesh.org"*)
    ...
    ;;
  *"get configmap chaos-mesh-fault-contract"*)
    echo '{"data":{"ownerId":"playbuilder","allowedNamespaces":"app-dev","maxFaultDurationSeconds":"60","maxFaults":"1","costBoundary":"existing-eks-compute"}}'
    ;;
...
PLATFORM_CHECK_BIN_DIR="$tmp_dir/bin" PLATFORM_FAKE_KUBECTL_LOG="$tmp_dir/kubectl.log" \
  bash "$root/scripts/chaos-mesh-readiness-check.sh" dev-playdevops chaos-mesh playbuilder app-dev 60 1
...
PLATFORM_CHECK_BIN_DIR="$tmp_dir/bin" PLATFORM_FAKE_KUBECTL_LOG="$tmp_dir/bad.log" \
  bash "$root/scripts/chaos-mesh-readiness-check.sh" dev-playdevops chaos-mesh playbuilder app-dev 60 1 >/dev/null 2>&1
status=$?
set -e

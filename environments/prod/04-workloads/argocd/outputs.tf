...
output "workload_ownership" {
  description = "Canonical ownership metadata for this untaggable workload root"
  value       = terraform_data.workload_ownership.input
}
...
output "bootstrap_application" {
  ...
  value       = var.enable_bootstrap ? "mini-commerce-${var.environment}-bootstrap" : null
}
...
output "verification_commands" {
  ...
  value       = <<-EOT
    ...
    kubectl -n argocd get application mini-commerce-${var.environment}-bootstrap
  EOT
}

...
run "ch23_enables_managed_snapshot_controller_and_class" {
  ...
  variables {
    ...
    volume_snapshot_class_name        = "mini-commerce-ebs-snapshots"
  }
  ...
  assert {
    condition = (
      yamldecode(kubectl_manifest.volume_snapshot_class["mini-commerce-ebs-snapshots"].yaml_body).driver == "ebs.csi.aws.com" &&
      yamldecode(kubectl_manifest.volume_snapshot_class["mini-commerce-ebs-snapshots"].yaml_body).deletionPolicy == "Retain"
    )
    error_message = "The platform VolumeSnapshotClass must use the EBS CSI driver and Retain snapshots."
  }
}

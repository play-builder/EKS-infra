...
override_data {
  ...
  values = { outputs = {
    cluster_name                       = "dev-playdevops-eks"
    cluster_arn                        = "arn:aws:eks:ap-northeast-2:123456789012:cluster/dev-playdevops-eks"
...
run "platform_exports_five_protected_planes" {
  command = apply
  assert {
    condition     = toset(keys(output.audit_log_groups)) == toset(["control_plane", "vpc_flow", "application", "performance", "waf"]) && alltrue([for group in output.audit_log_groups : group.kms_key_arn == "arn:aws:kms:ap-northeast-2:123456789012:key/11111111-2222-3333-4444-555555555555"]) && output.audit_log_groups.application.retention_days == 30 && output.audit_log_groups.performance.retention_days == 30 && output.audit_log_groups.waf.retention_days == 30 && output.waf_log_group_arn == "arn:aws:logs:ap-northeast-2:123456789012:log-group:aws-waf-logs-dev-playdevops"

...
run "ch03_baseline" {
  ...
  assert {
    ...
    error_message = "Ch03 must install only External Secrets among just-in-time controllers."
  }
}
...
run "ch12_enables_only_reloader" {
  command = plan
  ...
  assert {
    condition = (
      length(module.external_secrets) == 1 &&
      length(module.reloader) == 1 &&
      module.k6_operator.enabled == false &&
      module.amp_alerting.enabled == false
    )
    error_message = "Ch12 must add Reloader without enabling k6 or alerting."
  }
}

run "ch16_enables_dev_load_and_alerting" {
  command = plan
  ...
  variables {
    ...
    enable_amp          = true
    enable_amp_alerting = true
  }

  assert {
    condition = (
      length(module.reloader) == 1 &&
      module.k6_operator.enabled == true &&
      module.amp_alerting.enabled == true
    )
    error_message = "Ch16 must explicitly enable the Dev k6 and AMP alerting resources."
  }
}

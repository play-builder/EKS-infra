...
variables {
  recovery_cluster_name     = "rebuilt"
  restore_time              = "2026-09-01T00:00:00Z"
...
override_module {
  target  = module.recovery_secrets
  outputs = { application_credentials = { database = { name = "recovery-commerce-target/mini-commerce/database", arn = "arn:aws:secretsmanager:us-east-1:123456789012:secret:recovery-commerce-target/mini-commerce/database-abcdef" }, migration = { name = "recovery-commerce-target/mini-commerce/migration", arn = "arn:aws:secretsmanager:us-east-1:123456789012:secret:recovery-commerce-target/mini-commerce/migration-abcdef" } }, secrets = {} }
}
override_data {
  target = data.terraform_remote_state.source
  values = { outputs = { database_contract = { schemaVersion = "platform.database/v1", accountId = "123456789012", region = "us-east-1", identifier = "commerce-source", root = { stateKey = "prod/03-database/terraform.tfstate", stateBucket = "prod-state" } } } }
}
override_data {
  target = data.terraform_remote_state.source_cluster
  values = { outputs = { cluster_arn = "arn:aws:eks:us-east-1:123456789012:cluster/source", oidc_provider = "oidc.eks.us-east-1.amazonaws.com/id/OLD" } }
}
run "rejects_source_identifier" {
  command = plan
  variables { identifier = "commerce-source" }
  expect_failures = [terraform_data.identity]
}
run "rejects_same_cluster" {
  command = plan
  override_data {
    target = data.aws_eks_cluster.recovery
    values = { arn = "arn:aws:eks:us-east-1:123456789012:cluster/source", identity = [{ oidc = [{ issuer = "https://oidc.eks.us-east-1.amazonaws.com/id/OLD" }] }] }
  }
  expect_failures = [terraform_data.identity]
}
run "recovery_state_contract" {
  command = apply
  assert {
    condition     = output.database_contract.root.stateKey == "recovery/03-database/terraform.tfstate" && output.database_contract.applicationCredentials.database.name == "recovery-commerce-target/mini-commerce/database"
    error_message = "Recovery must use isolated state and credential shells."
  }
}

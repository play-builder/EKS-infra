...
pb_require_file "$review_history"
[[ "$request_identity" =~ [^[:space:]] && "$request_identity" != pending ]] || \
  pb_fail SAVED_PLAN_APPROVAL_HISTORY_INVALID
[[ "$run_id" =~ ^[1-9][0-9]*$ ]] || pb_fail SAVED_PLAN_APPROVAL_HISTORY_INVALID
...
jq -e --arg requester "$request_identity" '.requestIdentity == $requester' \
  "$manifest" >/dev/null || pb_fail SAVED_PLAN_REQUEST_IDENTITY_MISMATCH
...
' "$review_history" 2>/dev/null) || pb_fail SAVED_PLAN_APPROVAL_HISTORY_INVALID
...
approval_sha=$(pb_raw_sha256_file "$artifact_dir/.approval-evidence.json.tmp")

...
[[ $# -eq 6 ]] || pb_fail 'usage: prod-bootstrap-check.sh <rendered-application.json> <dev-deployment.json> <dev-slo.json> <dev-ready.json> <design-decision.json> <estimate-decision.json>' 64
application=$1
deployment=$2
slo=$3
ready=$4
design=$5
estimate=$6
for name in OWNER_ID AWS_REGION AWS_ACCOUNT_ID; do [[ -n "${!name:-}" ]] || pb_fail "$name is required" 64; done
pb_validate_region "$AWS_REGION"
pb_validate_account "$AWS_ACCOUNT_ID"
for file in "$application" "$design" "$estimate"; do pb_require_file "$file"; done
pb_assert_canonical_utc_seconds "$design" 'Prod design decision timestamps' \
  '["issuedAt"]' '["expiresAt"]'
pb_assert_canonical_utc_seconds "$estimate" 'Prod estimate decision timestamps' \
  '["issuedAt"]' '["expiresAt"]'
PLATFORM_CHECK_DETAIL_ONLY=true bash "$SCRIPT_DIR/dev-ready-check.sh" "$deployment" "$slo" "$ready" >/dev/null

pb_assert_json "$application" '
...
validate_decision() {
  ...
  PREFLIGHT_STAGE="$stage" PREFLIGHT_GRADE="$expected_grade" pb_assert_json "$file" '
    (if $ENV.PREFLIGHT_STAGE == "design" then
      ...
      .schemaVersion == "playbuilder.prod-preflight/v1" and
      (.bindings | keys == ["capacityInputSha256","devDeploymentSha256","devReadySha256","devSloSha256","previousDecisionSha256","savedPlanSha256"])
    else
      keys == ["accountId","bindings","decision","evidenceGrade","expiresAt","finops","issuedAt","ownerId","region","schemaVersion","stage"] and
      .schemaVersion == "playbuilder.prod-preflight/v2" and
    ...
    .ownerId == $ENV.OWNER_ID and .accountId == $ENV.AWS_ACCOUNT_ID and .region == $ENV.AWS_REGION and
...
[[ -z "${PLATFORM_CHECK_BIN_DIR:-}" ]] || estimate_grade=STATIC
validate_decision "$estimate" estimate "$estimate_grade"

for name in FINOPS_CONTRACT_JSON PLATFORM_INSTANCE_ID; do [[ -n "${!name:-}" ]] || pb_fail "$name is required" 64; done
pb_require_file "$FINOPS_CONTRACT_JSON"
pb_assert_canonical_utc_seconds "$estimate" 'FinOps observation timestamps' '["finops","observedAt"]' '["finops","expiresAt"]'
finops_grade=CLOUD_RUNTIME
[[ "$estimate_grade" != STATIC ]] || finops_grade=LOCAL_VERIFIED
jq -e --arg grade "$finops_grade" --arg sha "$(pb_sha256_file "$FINOPS_CONTRACT_JSON")" \
...
' "$estimate" >/dev/null || pb_fail 'FINOPS_CONFIGURATION_EVIDENCE_BINDING_MISMATCH'

deployment_sha=$(pb_sha256_file "$deployment")
slo_sha=$(pb_sha256_file "$slo")
ready_sha=$(pb_sha256_file "$ready")
design_sha=$(pb_sha256_file "$design")
...
' "$design" >/dev/null || pb_fail 'DESIGN_EVIDENCE_BINDING_MISMATCH'
...
' "$estimate" >/dev/null || pb_fail 'ESTIMATE_EVIDENCE_BINDING_MISMATCH'

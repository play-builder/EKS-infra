...
slo=$2
ready=$3
for file in "$deployment" "$slo" "$ready"; do pb_require_file "$file"; done
...
' 'invalid playbuilder.dev-deployment/v1 evidence'
...
' 'invalid or expired playbuilder.dev-slo/v1 evidence'

pb_assert_json "$ready" '
  def nonblank: type == "string" and test("[^[:space:]\uFEFF]");
  def canonical_ecr_repository:
    capture("^(?<accountId>[0-9]{12})\\.dkr\\.ecr\\.(?<region>ap-northeast-2|us-east-1)\\.amazonaws\\.com/(?<name>[a-z0-9]+(?:[._/-][a-z0-9]+)*)$") as $repository |
    $repository.accountId == $ENV.AWS_ACCOUNT_ID and $repository.region == $ENV.AWS_REGION and
  ...
  (
    (.workflow.runUrl |
      capture("^https://github\\.com/(?<repository>[^/\\s]+/mini-commerce)/actions/runs/(?<url_run_id>[0-9]+)$")) as $run_url |
...
' 'invalid, stale, or aliased playbuilder.dev-ready/v1 evidence'

jq -en --slurpfile deployment "$deployment" --slurpfile slo "$slo" --slurpfile ready "$ready" '
  ($deployment[0]) as $d | ($slo[0]) as $s | ($ready[0]) as $r |
  ($r.workflow.runUrl |
    capture("^https://github\\.com/(?<repository>[^/\\s]+/mini-commerce)/actions/runs/[0-9]+$").repository) as $workflow_repository |
...
' >/dev/null || pb_fail 'DEV_READY_IDENTITY_MISMATCH'

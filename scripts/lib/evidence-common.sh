...
pb_require_file() {
  [[ -f "$1" ]] || pb_fail "file not found: $1" 66
  jq -e . "$1" >/dev/null 2>&1 || pb_fail "invalid JSON: $1" 65
}

pb_validate_region() {
  [[ "$1" == "ap-northeast-2" || "$1" == "us-east-1" ]] || \
    pb_fail "UNSUPPORTED_REGION: ${1:-<empty>}" 64
}

pb_validate_account() {
  [[ "$1" =~ ^[0-9]{12}$ ]] || pb_fail "invalid AWS account ID" 64
}
...
pb_assert_canonical_utc_seconds() {
  ...
  for path in "$@"; do
    value=$(jq -er --argjson path "$path" 'getpath($path) | select(type == "string")' "$file") || \
      pb_fail "invalid canonical UTC seconds timestamp: $label"
    pb_assert_canonical_utc_seconds_value "$value" "$label"
  done
}

pb_assert_canonical_utc_seconds_value() {
  local value=$1 label=$2
  jq -en --arg value "$value" '
    $value |
    test("^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}Z$") and
    ((try (fromdateiso8601 | todateiso8601) catch "") == .)
  ' >/dev/null || pb_fail "invalid canonical UTC seconds timestamp: $label"
}
...
pb_file_mode() {
  ...
  pb_fail "unable to read file mode: $file"
}

pb_assert_file_mode() {
  local file=$1 expected=$2 actual
  actual=$(pb_file_mode "$file")
  [[ "$actual" == "$expected" ]] || \
    pb_fail "unexpected file mode for $file: expected $expected, got $actual"
}

pb_write_json() {
  local output=$1 payload=$2 output_dir tmp_file
  output_dir=$(dirname -- "$output")
  [[ -d "$output_dir" ]] || pb_fail "output directory not found: $output_dir" 66
  tmp_file=$(mktemp "$output.tmp.XXXXXX")
  printf '%s\n' "$payload" >"$tmp_file"
  jq -e . "$tmp_file" >/dev/null || {
    rm -f -- "$tmp_file"
    pb_fail "refusing to write invalid JSON"
  }

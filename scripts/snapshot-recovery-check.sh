...
[[ $# -eq 1 || $# -eq 2 ]] || pb_fail 'usage: snapshot-recovery-check.sh <snapshot-recovery.json> [snapshot-quiesce.json]' 64
recovery=$1
pb_require_file "$recovery"
pb_assert_eks_cluster_arn \
  "$(jq -r '.clusterArn // empty' "$recovery")" \
  "$(jq -r '.region // empty' "$recovery")"

pb_assert_json "$recovery" '
  ...
  .schemaVersion == "playbuilder.snapshot-recovery/v1" and .evidenceGrade == "CLOUD_RUNTIME" and .environment == "dev" and
...
if [[ $# -eq 2 ]]; then
  PLATFORM_CHECK_DETAIL_ONLY=true bash "$SCRIPT_DIR/snapshot-quiesce-check.sh" "$2" >/dev/null
  ...
  ' "$recovery" >/dev/null || pb_fail 'SNAPSHOT_RECOVERY_IDENTITY_MISMATCH'
fi
